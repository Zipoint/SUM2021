...
static VOID MH5_UnitInit( mh5UNIT_PIG *Uni, mh5ANIM *Ani )
{
  ...
  MH5_RndPrimsLoad(&Uni->Pig, "BIN/MODELS/pig.g3dm");
...
static VOID MH5_UnitResponse( mh5UNIT_PIG *Uni, mh5ANIM *Ani )
{
  ...
  if (Uni->PigLoc.X > -13.8 && Uni->PigLoc.Z > -15.8 && Uni->PigLoc.X < 13.8 && Uni->PigLoc.Z < 15.8)
  {
    if (Ani->JPov == 0)
      Uni->PigLoc = VecAddVec(Uni->PigLoc, VecMulNum(Uni->PigDir, Ani->GlobalDeltaTime * 20));
    ...
    if (Ani->JPov == 6)
    {
      Uni->PigRight = VecSet(Uni->PigRight.X, Uni->PigRight.Y, Uni->PigRight.Z);
      Uni->PigLoc = VecAddVec(Uni->PigLoc, VecMulNum(Uni->PigRight, -Ani->GlobalDeltaTime * 20));
    }
    if (Ani->JPov == 2)
    {
      Uni->PigRight = VecSet(Uni->PigRight.X, Uni->PigRight.Y, Uni->PigRight.Z);
    ...
    else if (Uni->PigLoc.X < -13.8)
      Uni->PigLoc.X = -13.7;
    else if (Uni->PigLoc.Z < -15.8)
      Uni->PigLoc.Z = -15.7;
    else if (Uni->PigLoc.X > 13.8)
      Uni->PigLoc.X = 13.7;
    else if (Uni->PigLoc.Z > 15.8)
...
{
  MH5_RndPrimsDraw(&Uni->Pig, MatrMulMatr(MatrScale(VecVec1(0.04)), MatrTranslate(Uni->PigLoc)));
